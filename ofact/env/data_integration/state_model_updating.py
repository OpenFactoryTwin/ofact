...
class StateModelUpdating:
    ...
    def store_into_state_model(self):
    ...
    def _get_key_value_pair(self, object_attr_types, key, value):
        ...
        if isinstance(value, dict) and "dict" not in object_attr_types[key]:
            ...
            if cache_key in self.cache:
                ...
                if "Order" in object_attr_types[key]:
                ...
                elif "Part" in object_attr_types[key]:
                    object_dict_to_create_attr = \
                    ...
                    raise NotImplementedError(object_attr_types, key, value)
            ...
            if not value:
                return None, None
            # case parts_involved
            elif [elem for elem in value if isinstance(elem, dict)] and "dict" not in object_attr_types[key]:

                if "parts_involved" in key:
                    new_values = list(set(self._create_new_values(dict_, "Part")
                                          if isinstance(dict_, dict) else dict_
                                          for dict_ in value
                                          if dict_ == dict_))
                    ...
                    new_values = list(set(self._create_new_values(dict_, "PassiveMovingResource")
                                          if isinstance(dict_, dict) else dict_
                                          for dict_ in value
                                          if dict_ == dict_))
