...
from ofact.twin.state_model.entities import ActiveMovingResource, Warehouse, ConveyorBelt
...
class MapperMethods:
    ...
    def __init__(self, digital_twin: StateModel, agents: Agents, change_handler: ChangeHandler):
        ...
        :param digital_twin: digital_twin model object
        :param agents: the agents_model
        :param change_handler: interface between the agents, digital_twin and environment
        """
    ...
    @classmethod
    def determine_robot(cls, x, idx=0):  # ToDo
        return None
        # robot = Robot(identification=None,
        #               name=list(ROBOTS.keys())[idx],
        #               serial=list(ROBOTS.values())[idx]['serial'],
        #               event_loop=asyncio.get_event_loop())
        #
        # return robot

    @classmethod
    def determine_connect(cls, x):
        return None
        # return Robot.connect

    @classmethod
    def determine_initialize_robot(cls, x):
        return None
        # return Robot.initialize_robot_position

    @classmethod
    def determine_process_step_provider(cls, x):
        # ToDo: process_step_provider - dynamisch möglich - eigentlich sollte dies in den Agenten geschehen
        return {}
    ...
    def determine_entity_provider(cls, x) -> dict[EntityType: list[Resource]]:
        ...
        if x:
            resources = cls.get_resources()
            warehouses = [resource_ for resource_ in resources if isinstance(resource_, Warehouse)]
...
class MappingAgents(Mapping):
    ...
    object_columns = {
        'digital_twin': None,
        'agents': MapperMethods.determine_agents,
        ...
        'robot': MapperMethods.determine_robot,
        'connect': MapperMethods.determine_connect,
        'initialize_robot': MapperMethods.determine_initialize_robot,
        'process_step_provider': MapperMethods.determine_process_step_provider,
    ...
    to_be_defined_object_columns = {
        'digital_twin': MapperMethods.determine_digital_twin,
        'agents': MapperMethods.determine_agents,
        'change_handler': MapperMethods.determine_change_handler,
        'address_book': convert_str_to_dict,
        'processes': convert_str_to_dict,
...
class AgentsObjects(metaclass=Singleton):
    ...
    def __init__(self, path, digital_twin: StateModel, empty_agents_model: Agents, mapping_class=MappingAgents,
        ...
        :param path: ToDo
        :param digital_twin:
        :param empty_agents_model:
        :param order_agent_amount: for a subsequent adaption of the number order agents in the system usable
        """
        super().__init__()
        self.path = path
    ...
    # ==== Excel-Export =====================================================================================
    # ToDo: WHAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAT? - didn't make sense
    def agents_to_df_for_excel_export(self):

        agents_order_agents_list = list(self.order_agents.keys())
        agents_agent_type = [agents_order_agents_list[i][0] for i in range(0, len(agents_order_agents_list))]
        agents_order_agents_name = [self.order_agents.get(i).name for i in agents_order_agents_list]
        agents_order_agents_password = [self.order_agents.get(i).password_xmpp_server for i in
                                        agents_order_agents_list]
        agents_order_agents_ip = [self.order_agents.get(i).ip_address_xmpp_server for i in agents_order_agents_list]
        agents_order_agents_digital_twin = [self.order_agents.get(i).digital_twin for i in agents_order_agents_list]

        address_book_l1 = [list(self.order_agents.get(i).address_book.values()) for i in agents_order_agents_list]
        agents_order_agents_address_book = [
            [dict(zip(address_book_l1[j][i], list(address_book_l1[j][i]))) for i in range(len(address_book_l1[j]))]
            for j in range(len(address_book_l1))]
        # agents_order_agents_processes = [self.order_agents.get(i).processes for i in agents_order_agents_list]

        agents_order_agents_processes = []
        for agent in agents_order_agents_list:
            if hasattr(self.order_agents.get(agent), 'processes'):
                agents_order_agents_processes.append(self.order_agents.get(agent).processes)
            else:
                agents_order_agents_processes.append(None)

        agents_order_agents_resource = []
        for gent in agents_order_agents_list:
            if hasattr(self.order_agents.get(agent), 'resource'):
                agents_order_agents_resource.append(self.order_agents.get(agent).resource)
            else:
                agents_order_agents_resource.append(None)

        # agents_order_agents_resource = [self.order_agents.get(i).resource for i in agents_order_agents_list]

        return agents_order_agents_list
...
if __name__ == '__main__':
    ...
    # we need one path to the Excel file containing a sheet for all the following: plant, process and order

    digital_twin_path = os.path.join(ROOT_PATH, 'projects\\bicycle_world\\models\\bicycle_world.xlsx')

    digital_twin_objects = StaticModelGenerator(digital_twin_path)
    digital_twin = digital_twin_objects.get_digital_twin()
    all_digital_twin_objects = digital_twin_objects.get_all_objects_digital_twin()
