    ...
    for key in class_.object_columns:
        if key in params:
            if not isinstance(params[key], list) and not isinstance(params[key], dict):
                if params[key] not in {np.nan, np.NaN, None} and pd.notna(params[key]):
                    return False
    return True
...
def _string_to_class_instance(class_parameters, mapping_class, class_name=None):
    ...
    attributes_of_class = list(inspect.signature(mapping_class.mappings[class_name]).parameters)
    ...
    attributes_of_class_required = \
        [True if type(elem.default) == type else False
         for elem in list(inspect.signature(mapping_class.mappings[class_name]).parameters.values())]
